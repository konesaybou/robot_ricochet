...
public class AStar extends ModelEcoutable {
	...
	public ArrayList<Noeud> solveur() {
		...
		while (!(courant.getRobot(coulRobPrincipal).getPosition().equals(arrivee)) && (!openList.isEmpty())) {
			...
			// r�cup�ration des adjacents du noeud courant
	...
	/**
	 * construit le chemin une fois la solution trouv�e
	 * 
	...
	public void construireChemin(Noeud destination) {
		...
		for (int i = chemin.size() - 1; i > 0; i--) {
			for (int j = 0; j < 4; j++) {
				...
				if (!chemin.get(i).getListeRobot()[j].getPosition()
						.equals(chemin.get(i - 1).getListeRobot()[j].getPosition())) {
					// calcul de la direction � prendre
					if (chemin.get(i - 1).getListeRobot()[j].getPosition().getligne() < chemin.get(i).getListeRobot()[j]
							.getPosition().getligne()) {
						dr = Direction.HAUT;
					}
					if (chemin.get(i - 1).getListeRobot()[j].getPosition().getligne() > chemin.get(i).getListeRobot()[j]
							.getPosition().getligne()) {
					...
					if (chemin.get(i - 1).getListeRobot()[j].getPosition().getcol() < chemin.get(i).getListeRobot()[j]
							.getPosition().getcol()) {
						dr = Direction.GAUCHE;
					}
					if (chemin.get(i - 1).getListeRobot()[j].getPosition().getcol() > chemin.get(i).getListeRobot()[j]
					...
					solution.add(new Solution(chemin.get(i - 1).getListeRobot()[j].getCouleur(), dr,
							chemin.get(i - 1).getListeRobot()[j].getPosition()));
				}
			}
		}
		for (int i = 0; i < solution.size(); i++) {
			System.out.println(solution.get(i).getCoulRob() + " " + solution.get(i).getDirection() + " "
					+ solution.get(i).getPosition());
		}
		fireChangement();
	}
